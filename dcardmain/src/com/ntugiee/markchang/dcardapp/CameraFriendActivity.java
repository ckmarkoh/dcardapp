...
public class CameraFriendActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.camera_friend);
...
//							Toast.makeText(SendActivity.this, "send success", Toast.LENGTH_LONG).show();
					...
					Toast.makeText(CameraFriendActivity.this, "send failed, error:"+error, Toast.LENGTH_LONG).show();
				}
				//	Toast.makeText(SendActivity.this, "post success", Toast.LENGTH_LONG).show();
				//}
			}
			else{
				Log.d("error","http response"+response.getStatusLine().getStatusCode() );
			}
		} catch (Exception e) {
			
			Log.d("error",e.getMessage().toString());

			Toast.makeText(CameraFriendActivity.this, e.getMessage().toString(), Toast.LENGTH_LONG).show();
			e.printStackTrace();
			}		
		...
		MCFNextButton.setOnClickListener(new View.OnClickListener() {
			...
			try {
				...
				if(response.getStatusLine().getStatusCode() == 200){
					String raw_result = EntityUtils.toString(response.getEntity());
					Toast.makeText(CameraFriendActivity.this, "送出成功", Toast.LENGTH_LONG).show();
